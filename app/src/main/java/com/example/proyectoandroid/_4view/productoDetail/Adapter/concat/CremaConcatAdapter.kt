...
import com.example.proyectoandroid.databinding.AdicionalCremaBinding
...
class CremaConcatAdapter(private val complementAdapter: ComplementCheckAdapter) : RecyclerView.Adapter<BaseConcatHolder<*>>(){

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseConcatHolder<*> {
        val itemBinding=
            AdicionalCremaBinding.inflate(LayoutInflater.from(parent.context),parent,false)
    ...
    private inner class ConcatViewHolder(val binding: AdicionalCremaBinding)
        : BaseConcatHolder<ComplementCheckAdapter>(binding.root){
        override fun bind(adapter: ComplementCheckAdapter) {
            binding.rvCrema.adapter=adapter
        }
    }
}